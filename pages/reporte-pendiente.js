...
const reporte = () => {
    ...
    const fetcher = () => axios('/api/pendiente').then(datos => datos.data)
    const { data, error, isLoading } = useSWR('/api/pendiente',fetcher,{refreshInterval: 100} )


  return (
    <>
    <LayoutAdmin pagina={`Reportes`}>
        <h1 className="text-2xl font-black text-center py-2">Reportes Pendientes</h1>
